...
select max(salary)as MAX_Salary from Employees -- 2 - task (easy task)

SELECT AVG(Age) AS AVG_Age FROM Employees -- 6-easy task


SELECT EmployeeID, COUNT(*) AS Employe_Count FROM Employees GROUP BY EmployeeID --7- easy task

SELECT EmployeeID, COUNT(*) AS EmployeeCount FROM Employees GROUP BY EmployeeID HAVING COUNT(*) > 5 -- 8-east task

SELECT COUNT(*) AS HREmployees FROM Employees WHERE DepartmentName = 'HR'--2-task middle

SELECT EmployeeID,  MAX(Salary) AS MaxSalary,  MIN(Salary) AS MinSalar FROM Employees GROUP BY EmployeeID --3- middle task

SELECT EmployeeID,  AVG(Salary) AS AverageSalary FROM Employees GROUP BY EmployeeID -- 4 task middle

SELECT EmployeeID, AVG(Salary) AS AverageSalary,  COUNT(*) AS EmployeeCount FROM Employees GROUP BY EmployeeID --5 task middle

SELECT EmployeeID,  AVG(Salary) AS AverageSalary FROM Employees GROUP BY EmployeeID HAVING AVG(Salary) > 60000 --9task middle

SELECT EmployeeID,  SUM(Salary) AS TotalSalary,  AVG(Salary) AS AvgSalary
FROM Employees
GROUP BY EmployeeID
HAVING AVG(Salary) > 65000--3 task hard
...
select MIN(price) as MIN_Price from Products -- 1-task(easy task)

SELECT COUNT(DISTINCT Category) AS UniqueCategories FROM Products -- 4-task (easy task)

select * from Products where 1=0

SELECT SUM(ProductID * Price) AS Total_sales FROM Products WHERE ProductID = 7 -- 5-easy task

SELECT Category, MIN(Price) AS Min_Price, MAX(Price) AS Max_Price FROM Products GROUP BY Category -- 8-easy task

SELECT Category,  AVG(Price) AS AveragePrice FROM Products GROUP BY Category HAVING AVG(Price) > 400 --6-task middle

SELECT Category,  AVG(Price) AS AvgPrice FROM Products GROUP BY Category HAVING AVG(Price) > 150--1-hard task
...
select COUNT(*) as Total_Count from Customers -- 3-task (easy task)
...
SELECT CustomerID, SUM(SaleAmount * ProductID) AS Total_Sales FROM Sales GROUP BY CustomerID --9-task easy


SELECT SaleID, SUM(SaleAmount) AS TotalSales, AVG(SaleAmount) AS AverageSales FROM Sales GROUP BY SaleID --1-middle task

SELECT YEAR(SaleDate) AS SaleYear, SUM(SaleAmount) AS TotalSales FROM Sales GROUP BY YEAR(SaleDate) ORDER BY SaleYear--7-task middle

SELECT CustomerID,  COUNT(*) AS OrderCount FROM Sales GROUP BY CustomerID HAVING COUNT(*) >= 3--8-task middle


SELECT  YEAR(OrderDate) AS SaleYear, MIN(Quantity) AS MinOrderQuantity,MAX(Quantity) AS MaxOrderQuantity
FROM Orders
GROUP BY YEAR(OrderDate)--hard task 6
